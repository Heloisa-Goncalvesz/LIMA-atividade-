# Heloísa Gonçalves Silva

Campinas - SP | 
[helo.goncalves0515@gmail.com](mailto:helo.goncalves0515@gmail.com) | 
(19) 98319-4457  
[GitHub](https://github.com)

---

## Objetivo Profissional

Atuar como desenvolvedora de software, aplicando meus conhecimentos e desenvolvendo novas habilidades profissionais, visando outros ramos acadêmicos.

---

## Formação Acadêmica

* **Técnico em Desenvolvimento de Sistemas** — SENAI  
  *Previsão de conclusão: 2027*

* **Ensino Médio** — SESI  
  *Previsão de conclusão: 2027*

---

## Conhecimentos Técnicos

* **Linguagens de Marcação & Estilização:** HTML5, Markdown
* **Linguagens de Programação:** JavaScript
* **Ferramentas & Controle de Versão:** Git, GitHub, VS Code
* **Banco de Dados:** SQL Server

---

## Projetos Desenvolvidos

* Programação em JavaScript
* Programação de Vetores
* Programação de Laços de Repetição
* Projeto Scrum

---

## Cursos Complementares

* **Ética na Inteligência Artificial**
* **Excel Básico**
* **Fundamentos da Inteligência Artificial**
* **Língua Inglesa CPM** *(Carga horária: 156 horas)*

---

## Competências 

* Resolução de problemas e lógica de programação
* Trabalho em equipe e comunicação eficientes
* Gestão de tempo e organização pessoal
* Responsabilidade e proatividade

---

## Experiência Profissional

* Em busca da primeira oportunidade profissional na área de tecnologia / Em transição de carreira.